...
dd if=/dev/zero of=kernel.img count=10 bs=1048576

# Make the partition table, partition and set it bootable.
parted --script kernel.img mklabel msdos mkpart p ext2 1 10 set 1 esp on set 1 boot on p

# Map the partitions from the image file and find where loopback will be
looppart=`kpartx -l kernel.img | awk -e '{ print $1; exit }'`
loopdev=`kpartx -l kernel.img | awk -e '{ print $5; exit }'`
kpartx -av kernel.img
...
# chown $LOGNAME:$LOGGROUP -R build kernel.img